...
class Configuration(models.Model):
    ...
    bhk_type = models.CharField(
        max_length=50,
        help_text="Example: 2 BHK, 3 BHK"
    )

    area_sqft = models.PositiveIntegerField(
        verbose_name="Carpet Area (Sq.ft)",
        help_text="Enter numeric carpet area in square feet"
    )

    price_in_rupees = models.PositiveIntegerField(
        verbose_name="Price (₹)",
        help_text="Enter full price in rupees (example: 27500000)"
    )

    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        ordering = ["bhk_type", "area_sqft"]
        verbose_name = "Configuration"
        verbose_name_plural = "2. Configurations"
        indexes = [
            models.Index(fields=["bhk_type"]),
            models.Index(fields=["area_sqft"]),
            models.Index(fields=["price_in_rupees"]),
        ]

    def __str__(self):
        return f"{self.project.project_name} | {self.bhk_type} | {self.area_sqft} sqft"

    # ✅ Helper: formatted price (₹ Cr / Lakh)
