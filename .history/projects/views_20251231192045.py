...
from utility.models import City, Locality , PropertyType , PossessionIn , ProjectAmenities , Bank
# Import related models for dropdowns
from .models import (
...
def search_projects(request):
    ...
    # ================= GET PARAMS =================
    category = request.GET.get("category")        # YES
    property_type = request.GET.get("property_type")  # YES
    city = request.GET.get("city")                # YES
    locality = request.GET.get("locality")        # YES
    location = request.GET.get("location")        # YES
    bhk = request.GET.get("bhk")                  # YES
    min_price = request.GET.get("min_price")      # YES
    max_price = request.GET.get("max_price")      # YES
    area = request.GET.get("area")                # YES
    sort = request.GET.get("sort")                # YES

    # ================= BASE QUERY =================
    projects = Project.objects.filter(active=True)  # YES

    if category:
        projects = projects.filter(category__iexact=category)  # YES
    ...
    if city:
        projects = projects.filter(city_id=city)  # YES

    if locality:
        projects = projects.filter(locality_id=locality)  # YES
    ...
    if bhk:
        ...
        )  # UPDATED (bhk_type se dynamic config)
    ...
    if min_price:
        projects = projects.filter(
            max_price_val__gte=int(min_price)
        )  # YES

    if max_price:
        projects = projects.filter(
            min_price_val__lte=int(max_price)
        )  # YES

    # ================= SORT =================
    if sort == "price_low":
        projects = projects.order_by("min_price_val")  # YES
    elif sort == "price_high":
        projects = projects.order_by("-min_price_val")  # YES
    elif sort == "latest":
        projects = projects.order_by("-create_at")  # YES
    elif sort == "possession":
    ...
    # ================= PAGINATION =================
    paginator = Paginator(projects, 9)  # YES
    page = request.GET.get("page")       # YES
    projects_page = paginator.get_page(page)  # YES
    ...
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        ...
        return JsonResponse({"html": html})  # YES

    # ================= NORMAL RESPONSE =================
    context = {
        "settings_obj": settings_obj,   # YES
        "projects": projects_page,      # YES
        "cities": City.objects.all(),   # YES
        "selected": request.GET,        # YES
    }
